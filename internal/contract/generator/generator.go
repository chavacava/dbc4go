...
func (fr *fileRewriter) rewrite(node ast.Node) (ast.Node, bool) {
	switch n := node.(type) {
	case *ast.FuncDecl:
		fd, newImports := fr.rewriteFuncDecl(n)
		println(">>>> newImports len", len(newImports))
		// fr.updateImports(newImports)
		return fd, true
	}
...
/*
func (fr *fileRewriter) updateImports(imports map[string]struct{}) {
	println(">>>> updating imports")
	for _, imp := range fr.imports {
		println(">>>> looking for", imp.Path.Value)
		_, ok := imports[imp.Path.Value]
		if !ok {
			continue
		}
		println(">>>> found!")
	...
	for k, _ := range imports {
		println(">>>> adding ", k)
		is := ast.ImportSpec{Path: astutils.NewStringLit(k), Comment: nil, EndPos: 1}
		fr.imports = append(fr.imports, &is)
	}
}
*/
...
func (fr *fileRewriter) rewriteFuncDecl(fd *ast.FuncDecl) (r *ast.FuncDecl, imports map[string]struct{}) {
	...
	contract := contract.NewFuncContract(fd, fr.file)
	...
	println(">>>> new contract Imports len", len(contract.Imports()))

	return fd, contract.Imports()
}
